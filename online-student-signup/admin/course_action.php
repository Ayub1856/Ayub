...
if(isset($_POST["action"]))
{
	...
	if($_POST["action"] == 'Add' || $_POST["action"] == "Edit")
	{
		...
		{
			$error_course_name = 'Course Unit Name is required';
			$error++;
		}
		else
		{
		...
		else
		{
			if($_POST["action"] == "Add")
			{
				...
				(course_name) 
				SELECT * FROM (SELECT :course_name) as temp 
				WHERE NOT EXISTS (
					SELECT course_name FROM tbl_course WHERE course_name = :course_name
				) LIMIT 1
				";
				$statement = $connect->prepare($query);
				if($statement->execute($data))
				{
					...
					else
					{
						$output = array(
							'error'					=>	true,
							'error_course_name'		=>	'Course Name Already Exists'
						);
					}
				}
			}
